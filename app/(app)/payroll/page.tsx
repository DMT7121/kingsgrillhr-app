  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <StatCard label="Tổng thu nhập" value="21.5tr" sub="Tháng 5/2026" tone="blue" />
        <StatCard label="Khấu trừ" value="1.7tr" tone="red" />
        <StatCard label="Thực nhận" value="19.8tr" tone="green" />
        <StatCard label="So tháng trước" value="+7%" sub="Tăng 1.3tr" tone="purple" />
      </div>

      <div className="grid md:grid-cols-2 gap-4 mb-6">
        <Card title={`Chi tiết phiếu lương — ${payrollData?.latest?.periodName || "Không có dữ liệu"}`}>
          {loading ? (
            ...
            <div className="space-y-2">
              {payrollData.latest.items.map((item) => (
              ...
              <div className="flex justify-between pt-3 mt-2 border-t-2 border-brand-100">
                ...
                <span className="text-lg font-extrabold text-brand-700">{formatMoney(payrollData.latest.netSalary)}</span>
              </div>
            </div>
          )}
        </Card>
        ...
        <Card title="Biểu đồ thu nhập">
          <div className="flex items-end gap-3 h-44">
            {[16.5, 17.2, 18.1, 19.2, 18.5, 19.8].map((v, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[10px] font-bold text-brand-600">{v}tr</span>
                <div className="w-full rounded-t-lg bg-gradient-to-t from-brand-600 to-brand-400" style={{ height: `${(v / 20) * 100}%` }} />
